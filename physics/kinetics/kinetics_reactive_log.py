...
def kinetic_block_rule(block):
    ...
    def k_FT_rule(block):
        return  pe.log(block.k_FT) - pe.log(k.k0_FT) == -k.E_FT*1e3/(k.R*block.parent_block().T)
    ...
    def alpha_rule(block):
        return 2*pe.log(block.alpha) - pe.log(1-block.alpha) == block.g0_FT*1e3/(k.R*block.parent_block().T)
    ...
    def k_WGS_rule(block):
        return pe.log(block.k_WGS) - pe.log(k.k0_WGS) == -k.E_WGS*1e3/(k.R*block.parent_block().T)
    block.k_WGS_con = pe.Constraint(rule=k_WGS_rule)

    def Ke_WGS_rule(block):
        return pe.log(block.Ke_WGS) - pe.log(5*block.parent_block().T**(k.s5_WGS/k.R)) == \
                (k.s1_WGS/block.parent_block().T + k.s2_WGS + k.s3_WGS*block.parent_block().T + k.s4_WGS*(block.parent_block().T**2))/k.R
